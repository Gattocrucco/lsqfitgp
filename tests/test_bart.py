...
@amark
@smark
def test_lower_upper_incr_maxd(sb, sbw, sa, a, b):
    """
...
@amark
@smark
def test_incr_alpha(sb, sbw, sa, a, b, u, md):
...
@bmark
@amark
@mark.parametrize('sb,sbw,sa', sum([[
    (gen.integers(0, 10, p), np.zeros(p, int), gen.integers(0, 10, p)),
    np.zeros((3, p), int),
] for p in plist], []))
...
@amark
@smark
def test_swap_ab(sb, sbw, sa, a, b, u, md):
