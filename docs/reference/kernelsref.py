...
are `r` or `r2` if the kernel is isotropic, `delta` if it is stationary, or `x`,
`y` for generic kernels, and with only the keyword arguments specific to the
kernel. All kernels also understand the general keyword arguments of
`CrossKernel` (or their specific superclass), while there are no positional
arguments when instantiating the kernel and the call signature of instances is
always `x`, `y`.

Example: the kernel `GammaExp` is listed as ``GammaExp(r, gamma=1)``. This means
...
.. warning::
   ...
   You may encounter problems with second derivatives for `CausalExpQuad`,
   `FracBrownian`, `NNKernel`, and with first derivatives too for `Wendland`
   (but only in more than one dimension). :class:`Color` stops working for
   :math:`n > 20`.
